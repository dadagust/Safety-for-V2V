from __future__ import annotations
import os, csv, argparse
from pathlib import Path

def main():
    ap = argparse.ArgumentParser(description="Create CSV from ASVspoof-like protocol file.")
    ap.add_argument("--protocol", required=True)
    ap.add_argument("--wav-dir", required=True)
    ap.add_argument("--out-csv", required=True)
    ap.add_argument("--label-col", type=int, default=-1)
    args = ap.parse_args()

    wav_dir = Path(args.wav_dir)
    idx = {}
    for p in wav_dir.rglob("*"):
        if p.suffix.lower() in [".wav", ".flac"]:
            idx[p.stem] = str(p)

    rows = []
    with open(args.protocol, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            parts = line.split()
            utt = parts[0]
            lab = parts[args.label_col].lower()
            if utt not in idx:
                continue
            label = 1 if lab == "bonafide" else 0
            rows.append((idx[utt], label))

    os.makedirs(os.path.dirname(args.out_csv) or ".", exist_ok=True)
    with open(args.out_csv, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["path", "label"])
        w.writerows(rows)

    print(f"Wrote {len(rows)} rows -> {args.out_csv}")

if __name__ == "__main__":
    main()
